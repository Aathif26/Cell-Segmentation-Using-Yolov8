...
class DataIngestion:
    ...
    def download_data(self)-> str:
        ...
        try:
            ...
            logging.info(f"Download data from {dataset_url} into file {zip_file_path}")
    ...
    def initiate_data_ingestion(self)-> DataIngestionArtificat:
        logging.info("Entered initiate_data_ingestion method of Data_Ingestion class")
        try:
            zip_file_path = self.download_data()
            feature_store_path = self.extract_zip_file(zip_file_path)

            data_ingestion_artificat = DataIngestionArtificat(
                data_zip_file_path= zip_file_path,
                feature_store_path= feature_store_path
            )

            logging.info("Excited initiate_data_ingestion method of Data_Ingestion class")
            logging.info(f"Data ingestion artifcat: {data_ingestion_artificat}")
